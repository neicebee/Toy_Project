...
use web_sys::{window, Event, MouseEvent}; // input 이벤트 타입, button 클릭 이벤트 타입, 창 임포트
use log::info;

use crate::app::{AUTH_STATE, IS_INITIAL_AUTH_CHECK_COMPLETE, JWT_TOKEN_KEY, USERNAME_KEY};
...
pub fn Login() -> impl IntoView {
    view! {
        { move || {
            info!("{}", IS_INITIAL_AUTH_CHECK_COMPLETE.get());
            // 1. 초기 인증 상태 확인 작업이 완료되지 않았을 경우
            if !IS_INITIAL_AUTH_CHECK_COMPLETE.get() {
                info!("Login Guard: Initial check not complete. Waiting...");
                return view! { <p>"Waiting for initial auth check..."</p> }.into_view();
            }
            
            // 2. 초기 인증 상태 확인 작업 완료 시, AUTH_STATE 값 확인
            info!("Login Guard: Initial check complete. Checking AUTH_STATE. Current value: {:?}", AUTH_STATE.get());
            if AUTH_STATE.get().is_some() { //
                info!("Login Guard (Internal): User authenticated, redirecting from login page.");
                return view! { <Redirect path="/todos"/> }.into_view(); // Redirect 컴포넌트 반환
            }
            
            // 3. 초기화 완료 및 비인증 상태일 시
            info!("Login Page: User not authenticated. Displaying login form.");
            // input field 값 상태 관리
            ...
            let on_register = move |ev: MouseEvent| {
                ev.prevent_default();
                navigate_2("/register", Default::default());
            };
            ...
            view! {
                ...
                // 로그인 요청 상태 표시
                <p>{login_status}</p>
                <button on:click=on_register>"Register"</button>
            }.into_view()
        }}
    }
}
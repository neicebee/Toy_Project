...
use crate::app::{AUTH_STATE, IS_INITIAL_AUTH_CHECK_COMPLETE};
    ...
    view! {
        { move || {
            ...
            if !IS_INITIAL_AUTH_CHECK_COMPLETE.get() {
                info!("Register Guard: Initial check not complete. Waiting...");
                return view! { <p>"Waiting for initial auth check..."</p> }.into_view();
            }
            
            // 2. 초기 인증 상태 확인 작업 완료 시, AUTH_STATE 값 확인
            info!("Register Guard: Initial check complete. Checking AUTH_STATE. Current value: {:?}", AUTH_STATE.get());
            if AUTH_STATE.get().is_some() { //
                info!("Register Guard (Internal): User authenticated, redirecting from login page.");

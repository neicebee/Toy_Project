...
def get_notices(text: str, Crawler, layout, final_categorized_data):
    output_file_name = f"{text}_categorized_notices.json"
...
if __name__=="__main__":
    # 메인 홈페이지 공지사항
    home_callback_url = 'https://www.hansung.ac.kr/bbs/hansung/143/artclList.do'
    home_notices_layout = '5Op5N6HXicqz2jqh4sxgz7ECT9MbeWxhVFwsIyGN%2F0c%3D'

    # 컴퓨터공학부 공지사항
    ce_callback_url = 'https://hansung.ac.kr/bbs/CSE/1248/artclList.do'
    ce_notices_layout = '5dDIBt1gXLAm40rxvKJ3cflabU8u4tWWDbOYcFOF2Xw%3D'

    # 공지사항 크롤링 객체 생성
    Crawler = get_html(home_callback_url)
    ...
    get_notices('home', Crawler, home_notices_layout, final_categorized_data)
    # 딕셔너리 초기화
    final_categorized_data['categories'] = {}
    final_categorized_data['notices_by_id'] = {}
    Crawler.change_url(ce_callback_url)
    get_notices('ce', Crawler, ce_notices_layout, final_categorized_data)